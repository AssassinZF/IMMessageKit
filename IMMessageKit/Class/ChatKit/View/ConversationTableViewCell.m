...
-(void)setConversationModel:(ConversationModel *)conversationModel{
    ...
    if (conversationModel.isTop == 1) {
        self.backgroundColor = XZRGB(0xf6f9fa);
    } else {
        self.backgroundColor = [UIColor whiteColor];
    }
    if (conversationModel.unReadCount > 0) {
        [self.unreadLabel setTitle:[NSString stringWithFormat:@"%d",conversationModel.unReadCount] forState:UIControlStateNormal];
    ...
    [_messageLabel setText:conversationModel.lastMsgString];
    [_usernameLabel setText:conversationModel.conversationName];
